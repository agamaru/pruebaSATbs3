...
class ConfiRedVoter extends Voter
{
    const VER = 'CONFIRED_VER';
    const CREAR = 'CONFIRED_CREAR';
    const EDITAR = 'CONFIRED_EDITAR';
    const ELIMINAR = 'CONFIRED_ELIMINAR';
    ...
    {
        if (!$subject instanceof ConfiRed) {
    ...
    private function puedeVer(ConfiRed $confiRed, TokenInterface $token, Usuario $usuario)
    {
        // todos pueden ver
        return true;
    }
    ...
    private function puedeEditar(ConfiRed $confiRed, TokenInterface $token, Usuario $usuario)
    {
        // sólo el administrador puede modificar
        return $this->decisionManager->decide($token, ['ROLE_ADMIN']);
    }

    private function puedeEliminar(ConfiRed $confiRed, TokenInterface $token, Usuario $usuario)
    {
