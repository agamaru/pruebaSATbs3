...
class DispositivoRedVoter extends Voter
{
    const VER = 'DISPOSITIVO_RED_VER';
    const CREAR = 'DISPOSITIVO_RED_CREAR';
    const EDITAR = 'DISPOSITIVO_RED_EDITAR';
    const ELIMINAR = 'DISPOSITIVO_RED_ELIMINAR';
    ...
    {
        if (!$subject instanceof DispositivoRed) {
    ...
    private function puedeVer(DispositivoRed $dispositivoRed, TokenInterface $token, Usuario $usuario)
    {
        // todos pueden ver
        return true;
    }
    ...
    private function puedeEditar(DispositivoRed $dispositivoRed, TokenInterface $token, Usuario $usuario)
    {
        // sólo el administrador puede modificar
        return $this->decisionManager->decide($token, ['ROLE_ADMIN']);
    }

    private function puedeEliminar(DispositivoRed $dispositivoRed, TokenInterface $token, Usuario $usuario)
    {
